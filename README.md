...
Интеграционный сервер, использующий Model Context Protocol (MCP), для предоставления доступа к данным и функциям Bitrix24 для больших языковых моделей (LLM) и других AI-агентов. LLM могут безопасно взаимодействовать с вашими CRM-данными (контакты, сделки, задачи), используя предопределенные "инструменты" и "ресурсы", предоставляемые этим сервером через стандартный протокол MCP.

## Ключевые Особенности

*   **Интеграция с Bitrix24:** Доступ к контактам, сделкам и задачам через Bitrix24 REST API.
...
### Установка для Bitrix24 Cloud (облако)

Ниже — минимальная последовательность для подключения MCP сервера к облачному Bitrix24:


   * Скопируйте URL вида:
     `https://<ваш-портал>.bitrix24.ru/rest/<user_id>/<secret>/`
2. **(Опционально) Установите пакет глобально:**

   ```bash
   uv pip install bitrix24-mcp
   ```

3. **Перед запуском задайте переменную окружения с вебхуком:**

   ```bash
   export BITRIX_WEBHOOK_URL="https://<ваш-портал>.bitrix24.ru/rest/<user_id>/<secret>/"
   ```
...
4. **Запустите MCP сервер:**
   ...
   ```bash
   uvx bitrix24-mcp
   # или, если пакет установлен как скрипт
   bitrix24-mcp
   ```

Если переменная `BITRIX_WEBHOOK_URL` не задана, сервер завершится с ошибкой валидации конфигурации.
...
*   `tool://create_task`
    *   **Описание:** Создание задачи в Bitrix24 (можно использовать для постановки заданий из MCP-клиента).
    *   **Параметры:** `title: str`, `description: str = ""`, `responsible_id: int | None = None`, `deadline: str | None = None`, `accomplices: list[int] | None = None`
    *   **Возвращает:** JSON-строка с результатом создания задачи и `task_id`.
*   `tool://list_tasks`
    *   **Описание:** Получение списка задач Bitrix24 по базовым фильтрам.
    *   **Параметры:** `limit: int = 20`, `filter_status: int | None = None`, `filter_responsible_id: int | None = None`
    *   **Возвращает:** JSON-строка со списком задач.
